...
Naively, we can use the Ford-Fulkerson algorithm, where we greedily choose paths at each step, flipping those paths that we used to move from $s$ to $t$. This achieves $O(mn)$ time, where $m$ is the path length from $s$ to $t$, and $n$ is the flow (???).
