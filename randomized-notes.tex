...
Naively, we can use the Ford-Fulkerson algorithm, where we greedily choose paths at each step, flipping those paths that we used to move from $s$ to $t$. This achieves $O(md)$ time, where $m$ is the path length from $s$ to $t$, and $d$ is the max flow from $s$ to $t$.

Suppose we are interested in the following two variants of this problem: all-pairs and single-source edge-connectivity. Then, using the algorithm described above, we can achieve $O(mdn^2)$ and $O(mdn)$ time, respectively, where $m,n$ are as above and any $d$ that upper bounds the max flow (for example, the maximum degree of a vertex) will work.
